...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .main-nav .grammar-nav-group{
        position:relative;
        display:inline-flex;
        align-items:center;
        flex:0 0 auto;
      }
      .main-nav .grammar-nav-main{
        display:inline-flex;
        align-items:center;
        gap:3px;
      }
      .main-nav .grammar-nav-toggle{
        width:30px;
        min-width:30px;
        height:36px;
        margin-left:-8px;
      ...
      .main-nav .grammar-nav-toggle:hover,
      .main-nav .grammar-nav-toggle:focus-visible{
        background:#fff3ee;
        color:var(--red-dark);
        outline:none;
      }
      .main-nav .grammar-nav-toggle svg{
        width:14px;
        height:14px;
        transition:transform .18s ease;
      }
      .main-nav .grammar-nav-group.open .grammar-nav-toggle svg,
      .main-nav .grammar-nav-group:hover .grammar-nav-toggle svg,
      .main-nav .grammar-nav-group:focus-within .grammar-nav-toggle svg{
        transform:rotate(180deg);
      }
      .main-nav .grammar-nav-dropdown{
        position:absolute;
        z-index:1000;
        top:calc(100% + 9px);
        left:50%;
        ...
        box-shadow:0 18px 42px rgba(80,42,28,.16);
        opacity:0;
        visibility:hidden;
        pointer-events:none;
        transition:opacity .16s ease,transform .16s ease,visibility .16s ease;
      }
      .main-nav .grammar-nav-dropdown::before{
        content:"";
        position:absolute;
        top:-6px;
        left:50%;
      ...
      .main-nav .grammar-nav-group.open .grammar-nav-dropdown,
      .main-nav .grammar-nav-group:hover .grammar-nav-dropdown,
      .main-nav .grammar-nav-group:focus-within .grammar-nav-dropdown{
        opacity:1;
        visibility:visible;
        pointer-events:auto;
        transform:translate(-50%,0);
      }
      .main-nav .grammar-nav-dropdown .nav-link{
        width:100%;
        min-height:42px;
      ...
      .main-nav .grammar-nav-dropdown .nav-link::before{
        content:"—";
        margin-right:9px;
        color:var(--gold);
        font-weight:900;
      }
      .main-nav .grammar-nav-dropdown .nav-link:hover,
      .main-nav .grammar-nav-dropdown .nav-link:focus-visible,
      .main-nav .grammar-nav-dropdown .nav-link.active{
        background:#fff5ef;
        color:var(--red-dark);
      }
      .main-nav .grammar-nav-group.has-sub-active > .grammar-nav-main{
        color:var(--red-dark);
        background:#fff4ef;
      }
      ...
      @media (max-width:820px){
        ...
        .main-nav .grammar-nav-group:hover:not(.open) .grammar-nav-dropdown{
          opacity:0;
          visibility:hidden;
          pointer-events:none;
          transform:translate(-50%,-8px);
  ...
  function installMenu() {
    ...
    dropdown.querySelectorAll(".nav-link").forEach((button) => {
      button.addEventListener("click", () => close(group));
    });
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape") close(group);
    });
