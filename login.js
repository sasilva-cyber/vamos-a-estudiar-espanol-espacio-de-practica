...
(function () {
  ...
  async function init() {
    ...
    if (!window.VAEAuth?.isConfigured?.()) {
      ...
      setStatus(loginStatus, "A Área do Estudiante está sendo preparada. O login será liberado assim que a autenticação estiver conectada.", "info");
      return;
    }
    ...
    try {
      ...
      if (session) {
        const next = params.get("next") === "aluno" ? "aluno/" : "aluno/";
        location.replace(`${window.VAEAuth.ROOT_PATH}${next}`);
        return;
      }
    } catch (_) {}
