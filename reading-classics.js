/* Prática Premium de leitura com literatura em espanhol. */
(function(){
  const READINGS = [
    {
      id:'lorca-la-guitarra', level:'A1', kind:'Poesía guiada', title:'La guitarra', author:'Federico García Lorca', work:'Poema del cante jondo',
    ...
    {
      id:'lorca-cancion-jinete', level:'A2', kind:'Poesía guiada', title:'Canción del jinete', author:'Federico García Lorca', work:'Canciones',
    ...
    {
      id:'lorca-baladilla-rios', level:'B1', kind:'Poesía y paisaje', title:'Baladilla de los tres ríos', author:'Federico García Lorca', work:'Poema del cante jondo',
    ...
    {
      id:'lorca-romance-sonambulo', level:'B2', kind:'Poesía simbólica', title:'Romance sonámbulo', author:'Federico García Lorca', work:'Romancero gitano',
    ...
    {
      id:'lorca-la-aurora', level:'C1', kind:'Poesía urbana', title:'La aurora de Nueva York', author:'Federico García Lorca', work:'Poeta en Nueva York',
    ...
    {
      id:'lorca-doncella-dialogo', level:'B2', kind:'Lectura teatral · archivo', title:'La doncella, el marinero y el estudiante', author:'Federico García Lorca', work:'Diálogos', uploadedFile:'la doncella.pdf',
      intro:'Leitura baseada no arquivo enviado para a plataforma. O foco é reconhecer ritmo de diálogo, jogo de palavras e caracterização das personagens.',
    ...
    {
      id:'cervantes-la-gitanilla', level:'B2', kind:'Novela corta · archivo', title:'La Gitanilla', author:'Miguel de Cervantes Saavedra', work:'Novelas ejemplares', uploadedFile:'Miguel de Cervantes Saavedra - La Gitanilla.pdf',
      intro:'Leitura guiada a partir do arquivo enviado. O percurso trabalha caracterização de Preciosa, vocabulário do espanhol clássico e contrastes construídos pelo narrador.',
      excerpt:'Salió la tal Preciosa la más única bailadora que se hallaba en todo el gitanismo, y la más hermosa y discreta que pudiera hallarse.',
      glossary:[['bailadora','dançarina'],['discreta','sensata, prudente; também inteligente no contexto clássico'],['crianza','criação, educação recebida'],['donaire','graça, elegância no modo de agir ou falar']],
      questions:[
        {q:'Como o narrador apresenta Preciosa nesse trecho?',options:['Como uma personagem sem habilidades','Como bailadora, hermosa e discreta','Como uma personagem idosa','Como uma personagem que evita a música'],a:1},
        {q:'No contexto da obra, “discreta” se aproxima de qual sentido?',options:['Silenciosa apenas','Sensata e inteligente','Desatenta','Assustada'],a:1},
        {q:'A caracterização inicial de Preciosa enfatiza principalmente…',options:['Suas qualidades e singularidade','Uma viagem marítima','Um conflito militar','Uma doença'],a:0}
      ]
    },
    {
      id:'cervantes-casamiento-enganoso', level:'C1', kind:'Novela ejemplar · archivo', title:'El casamiento engañoso', author:'Miguel de Cervantes Saavedra', work:'Novelas ejemplares', uploadedFile:'CASAMIENTO ENGAÑOSO.pdf',
      intro:'Leitura avançada a partir do arquivo enviado. A atividade explora ironia, espanhol clássico e o relato de Campuzano sobre casamento, engano e consequências.',
      excerpt:'No sabré decir si fue por amores, aunque sabré afirmar que fue por dolores, pues de mi casamiento, o cansamiento, saqué tantos en el cuerpo y en el alma.',
      glossary:[['flaqueza','fraqueza'],['traspiés','tropeços'],['convaleciente','pessoa em recuperação de uma doença'],['arrepentimiento','arrependimento']],
      questions:[
        {q:'O jogo entre “casamiento” e “cansamiento” produz principalmente um efeito de…',options:['Ironia sobre a experiência matrimonial','Descrição geográfica','Explicação científica','Elogio solene'],a:0},
        {q:'Campuzano afirma ter sofrido consequências…',options:['Somente econômicas','Somente profissionais','No corpo e na alma','Apenas durante uma viagem'],a:2},
        {q:'O tom do trecho combina relato pessoal com…',options:['Humor irônico e arrependimento','Manual de instruções','Notícia objetiva','Descrição de paisagem'],a:0}
      ]
    }
  ];
    ...
    node.innerHTML=`
      <div class="reading-classics-head">
        <div class="reading-classics-copy">
          <p class="student-section-kicker">Lectura exclusiva · Literatura en español</p>
          <h2 id="reading-classics-title">Práctica de lectura</h2>
          <p>Leia textos selecionados de diferentes autores, consulte o glossário e responda atividades de compreensão organizadas por nível. Seu melhor resultado fica salvo na sua conta.</p>
        </div>
      ...
      <div class="reading-classics-note">Seleções didáticas com foco em leitura guiada, vocabulário e interpretação. A coleção foi preparada para receber continuamente autores da Espanha, da América Latina e de outras tradições hispano-americanas.</div>`;
  ...
  function render(){
    ...
    list.forEach(item=>{
      ...
      const meta=document.createElement('div');meta.className='reading-classic-meta';
      [item.author,item.work,item.uploadedFile?'Arquivo enviado':null,'Glossário','3 questões'].filter(Boolean).forEach(v=>{const s=document.createElement('span');s.textContent=v;meta.appendChild(s);});
  ...
  function openReading(item){
    ...
    const sourceSuffix=item.uploadedFile?` · arquivo enviado: ${item.uploadedFile}`:' · trecho selecionado para prática didática';
    body.innerHTML=`
      <p class="reading-context">${item.intro}</p>
      <blockquote class="reading-excerpt">${item.excerpt}</blockquote>
      <small class="reading-source">${item.author} · ${item.work}${sourceSuffix}</small>
    ...
    track('premium_reading_open',{reading_id:item.id,level:item.level,title:item.title,author:item.author});
  }
  ...
  async function submitReading(){
    ...
    const correct=active.questions.reduce((sum,q,i)=>sum+(Number(answers[i])===q.a?1:0),0);
    const score=Math.round((correct/active.questions.length)*100);
    const completed=score>=70;
    result.textContent=completed?`${correct} de ${active.questions.length} corretas · ${score}%. Leitura concluída ✓`:`${correct} de ${active.questions.length} corretas · ${score}%. Revise o trecho e tente novamente.`;
    result.className=`reading-result ${completed?'success':'retry'}`;
    button.disabled=true;
    try{
      const session=await window.VAEAuth?.getSession?.();
      const userId=session?.user?.id;
      if(userId&&window.VAEAuth?.getClient){
        const previous=progress.get(active.id)||{attempts:0,best_score:0,completed:false};
        const best=Math.max(Number(previous.best_score||0),score);
        const finalCompleted=Boolean(previous.completed||completed);
        const supabase=window.VAEAuth.getClient();
        const {error}=await supabase.from('reading_classics_progress').upsert({
          user_id:userId,
          reading_id:active.id,
          attempts:Number(previous.attempts||0)+1,
          best_score:best,
          completed:finalCompleted,
          last_completed_at:finalCompleted?new Date().toISOString():null,
          updated_at:new Date().toISOString()
        },{onConflict:'user_id,reading_id'});
        if(error)throw error;
        progress.set(active.id,{reading_id:active.id,attempts:Number(previous.attempts||0)+1,best_score:best,completed:finalCompleted});
      }
      track('premium_reading_submit',{reading_id:active.id,level:active.level,score,completed,author:active.author});
      render();
    }catch(error){console.warn('Não foi possível salvar o progresso de leitura',error);}
    finally{button.disabled=false;}
  }

  function installFilters(){
    section?.querySelectorAll('[data-reading-level]').forEach(button=>button.addEventListener('click',()=>{
      activeLevel=button.dataset.readingLevel||'all';
      section.querySelectorAll('[data-reading-level]').forEach(b=>b.classList.toggle('active',b===button));
      render();
      track('premium_reading_filter',{level:activeLevel});
    }));
  }
